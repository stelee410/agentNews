...
function rowToType(r: {
  ...
  return { key: r.key, label_zh: r.label_zh, label_en: r.label_en, enabled: r.enabled === 1 };
}

export function listTypes(includeDisabled = true, d: Database.Database = getDb()): ContentType[] {
  const sql = includeDisabled
    ? "SELECT * FROM types ORDER BY key"
    : "SELECT * FROM types WHERE enabled = 1 ORDER BY key";
...
export function createType(t: ContentType, d: Database.Database = getDb()): void {
  d.prepare(
    "INSERT INTO types (key, label_zh, label_en, enabled) VALUES (?, ?, ?, ?)"
  ).run(t.key, t.label_zh, t.label_en, t.enabled ? 1 : 0);
}

export function updateType(
  key: string,
  patch: Partial<Pick<ContentType, "label_zh" | "label_en" | "enabled">>,
  ...
  d.prepare(
    "UPDATE types SET label_zh = ?, label_en = ?, enabled = ? WHERE key = ?"
  ).run(next.label_zh, next.label_en, next.enabled ? 1 : 0, key);
}
